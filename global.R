...
{  
  ...
  if (length(post_df))
  {
    ...
    if (energy == 'Elec')
    {
      post_df$Units = c('', 'kWh/sqft/day', 'kWh/sqft/day', 'kWh/sqft', 'kWh/sqft','F', 'F', 'Months')
    }else
    {
      post_df$Units = c('', 'BTU/sqft/day', 'BTU/sqft/day', 'BTU/sqft', 'BTU/sqft','F', 'F', 'Months')
    }
  }else
  {
    post_df = data.frame(NULL)
...
{ 
  ...
  post_output_df$Gross[5] = round(as.numeric(as.character(post_output_df$Unretrofit[5]))*as.numeric(area), 2)
  if (energy_n == 'Elec')
  {
    post_output_df$Gross.Unit[2:4] = c('kWh/day','kWh/day','kWh')
    post_output_df$Gross.Unit[5] = c('kWh/day')
  }else
  {
    post_output_df$Gross.Unit[2:4] = c('BTU/day','BTU/day','BTU')
    post_output_df$Gross.Unit[5] = c('BTU/day')
  }
  ...
  if(!is.na(post_output_df$Gross[5]))
  {
    post_output_df$Gross[5] = prettyNum(post_output_df$Gross[5], big.mark = ",", format = 'f')
  }
...
{ 
  ...
  binfo_df1 = t(data.frame(bdbid = binfo_df$bdbid, name = binfo_df$building_name))
  ...
  rownames(binfo_df1) = c('BDBID', 'Building Name')
...
{   
    ...
    #post_output_df = post_output_df[c(1,2,3,4,8,9,10,11),]
    return(post_output_df)
}
